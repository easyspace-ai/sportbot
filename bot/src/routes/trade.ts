...
import { resolvePolymarketTradingCredentials } from '../services/polymarketTrading';
import { refreshPolymarketBook } from '../services/polymarketWs';
import { sendTradeNotification } from '../telegram/notify';
import type { Allocation } from '../types';
import { createLogger } from '../logger';
import { recordPolymarketBuyFill } from '../services/riskService';
...
router.post('/api/trade', async (req: Request, res: Response) => {
  ...
  try {
    ...
    const message = err instanceof Error ? err.message : 'polymarket_not_configured';
    res.status(503).json({ error: 'polymarket_not_configured', message });
    return;
  }
  ...
  for (const allocation of plan.allocations) {
    ...
    try {
      ...
      if (
        allocation.platform === 'polymarket'
        && side === 'buy'
        && allocation.externalOutcomeId
        && outcome.externalId
      ) {
        const title = `${outcome.market.event.homeTeam} vs ${outcome.market.event.awayTeam}`;
        recordPolymarketBuyFill({
          outcomeId: allocation.outcomeId,
          tokenId: allocation.externalOutcomeId,
          title,
          sideLabel: outcome.label,
          fillOdds: allocation.expectedOdds,
          costUsd: allocation.size,
        }).catch((err) => {
          log.warn({ err, tradeId, outcomeId: allocation.outcomeId }, 'risk position record failed');
        });
      }
...
async function executeAllocation(allocation: Allocation): Promise<string> {
  ...
  throw new Error(`Unsupported platform: ${allocation.platform}`);
}
