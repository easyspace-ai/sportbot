...
export function startTelegramBot(): void {
  ...
  bot.use((ctx, next) => {
    const allowed = getTelegramAuthorizedChatId();
    if (!allowed || ctx.chat?.id.toString() !== allowed) {
      return;
    }
    return next();
  });
  ...
  bot.on('message:text', async (ctx) => {
    ...
    const allocs = plan.allocations
      .map((a) => `Polymarket: $${a.size.toFixed(2)} @ ${(a.expectedOdds * 100).toFixed(2)}%`)
      .join('\n');
